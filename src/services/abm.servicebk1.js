...
const ABMService = {
  ...
  async getDashboardStats(abmEmployeeCode) {
    ...
    // Fetch this ABM's own LY target — same 3-step fallback chain used in getTeamYearlyTargets
    // Step 1: ly_target_value on the FY26_27 assignment row (most reliable — set during FY26_27 setup)
    const cyAbmRows = await db('ts_yearly_target_assignments')
      .where({ assignee_code: abmEmployeeCode, fiscal_year_code: activeFy })
      .select('ly_target_value', 'ly_achieved_value');
    let myLyTarget  = cyAbmRows.reduce((s, a) => s + parseFloat(a.ly_target_value  || 0), 0);
    let myLyAchieved = cyAbmRows.reduce((s, a) => s + parseFloat(a.ly_achieved_value || 0), 0);
    // Step 2: cy_target_value on the FY25_26 assignment row
    if (!myLyTarget) {
      const lyAbmRows = await db('ts_yearly_target_assignments')
        .where({ assignee_code: abmEmployeeCode, fiscal_year_code: 'FY25_26' })
        .select('cy_target_value', 'ly_achieved_value');
      myLyTarget   = lyAbmRows.reduce((s, a) => s + parseFloat(a.cy_target_value   || 0), 0);
      myLyAchieved = lyAbmRows.reduce((s, a) => s + parseFloat(a.ly_achieved_value || 0), 0);
    }
    // Step 3: sum target_revenue from ts_product_commitments for FY25_26
    if (!myLyTarget) {
      const commitRow = await db('ts_product_commitments')
        .where({ employee_code: abmEmployeeCode, fiscal_year_code: 'FY25_26' })
        .sum('target_revenue as total')
        .first();
      myLyTarget = parseFloat(commitRow?.total || 0);
    }
    return { totalTbms: tbmCodes.length, totalCommitments: commitments.length,
      ...
      myYearlyTarget, myLyTarget, myLyAchieved };
  },
};
