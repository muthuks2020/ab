...
const ZBMService = {
  ...
  async getAbmSubmissions(zbmEmployeeCode, filters = {}) {
    // Only fetch commitments belonging to direct ABM reports — not TBMs/SRs below them.
    const directAbms = await getAbmSubordinates(zbmEmployeeCode);
    const subToAbmMap = {};
    for (const abm of directAbms) {
      subToAbmMap[abm.employee_code] = { abmCode: abm.employee_code, abmName: abm.full_name };
    }

    const subCodes = Object.keys(subToAbmMap);
    ...
    return rows.map((r) => {
      const formatted = formatCommitment(r);
      const abmInfo = subToAbmMap[formatted.employee_code] || {};
      return { ...formatted, abmCode: abmInfo.abmCode || '', abmName: abmInfo.abmName || '' };
    });
  },
  ...
  async bulkApproveAbm(submissionIds, zbmUser, comments = '') {
    const subs = await getAbmSubordinates(zbmUser.employeeCode);
